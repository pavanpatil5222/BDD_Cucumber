...
public class ChemistrySolutionSearch {
	...
	public boolean validatePatentThumbsUpThumbsDown(Controller Application, HashMap<String, String> input) {
		...
		try {
			...
			if (expThumbsDownHollowState.contains("nopad")) {
			...
			if (expThumbsUpHollowState.contains("nopad")) {
	...
	public boolean validateXDeleteOrRemoveKeyword(Controller Application, HashMap<String, String> input) {
		...
		try {
			...
			for (int i = 1; i < getListOfPills-2; i++) {
	...
	public boolean validateSearchIcon(Controller Application, HashMap<String, String> input) {
		...
		try {
			...
			Application.Logger.addStep("2.Edit Keywords in the search box",
					"Search Icon Must turn green/become enabled if something is edited in the search box");
			page_ChemicalSearchResults.clickOnPillBox(1);
			if (page_ChemicalSearchResults.getColorOfSerachIcon().equals("primary")) {
			...
			page_ChemicalSearchResults.clickOnKeywordPlusIcon();
			Application.Logger.endStep();
			...
			if (page_ChemicalSearchResults.getColorOfButtomSection().equals("rgba(0, 0, 0, 0)")) {
	...
	public boolean validatePdfNotAvailable(Controller Application, HashMap<String, String> input) {
		...
		try {
			...
			Application.Logger.addStep("3.Mousehover on Literature PDF", "PDF is not available message displays");
			Application.driver.switchTo().window(MainID);
			Application.waitTime(2);
			page_ChemicalSearchResults.clickOnChemExpHomePage();
			Application.waitTime(2);
			page_ChemicalSearchLandingPage.setSearchText("smiley");
			page_ChemicalSearchLandingPage.clickOnSearchIcon();
			Application.waitUntilFectchRecordProgressBarToDisappears();
			page_ChemicalSearchResults.clickOnTabLiterature();
			Application.waitUntilFectchRecordProgressBarToDisappears();
			if(page_ChemicalSearchResults.tabLiteratureSearch().mousehoverOnPdfNotAvailable()==true)
			{
				Application.Logger.addsubStep(LogStatus.PASS,"PDF is not available", false);
			}
			else {
				Application.Logger.addsubStep(LogStatus.FAIL, "PDF is available", true);
			}
			Application.Logger.endStep();
	...
	public boolean validateFilterDataForDifferentFields(Controller Application, HashMap<String, String> input) {
		...
		try {
			...
			page_ChemicalSearchResults.clickOnMoreFiltersDropdown();
	...
	public boolean validatePdfFromResultSet(Controller Application, HashMap<String, String> input) {
		boolean flag = true;
		try {
			Application.Logger.addStep("1.VERIFY PERFORMING THE SEARCH IN LANDING PAGE AND PDF GENERATION FOR PATENT TAB ",
					"USER SHOULD BE TAKEN TO THE RESULTS SET PAGE AND PDF SHOULD GET GENERATED");
			page_ChemicalSearchLandingPage = new Page_ChemicalSearchLandingPage(Application);
			page_ChemicalSearchResults = new Page_ChemicalSearchResults(Application);
			String searchText = input.get("searchtext");
			...
			if (!MainID.equals(ChildID))
				Application.Logger.addsubStep(LogStatus.PASS, "PDF window opened successfully for Patent tab", false);
			else
				Application.Logger.addsubStep(LogStatus.FAIL, "PDF window has not opened for Patent tab", true);
		Application.Logger.endStep();
		
		Application.Logger.addStep("2.VERIFY PERFORMING THE SEARCH IN LANDING PAGE AND PDF GENERATION FOR LITERATURE TAB ",
				"USER SHOULD BE TAKEN TO THE RESULTS SET PAGE AND PDF SHOULD GET GENERATED");
		Application.driver.switchTo().window(MainID);
		Application.waitTime(2);
		page_ChemicalSearchResults.clickOnTabLiterature();
		...
		String ChildID1 = al1.get(1);
		String ChildID2 = al1.get(2);
		System.out.println("----------------------------------");
		System.out.println("Main ID="+MainID);
		System.out.println("ChildID1"+ChildID1);
		System.out.println("ChildID2"+ChildID2);
		Application.driver.switchTo().window(ChildID1);

		if (!MainID.equals(ChildID2))
			Application.Logger.addsubStep(LogStatus.PASS, "PDF window opened successfully for Literature tab",
					false);
		else
			Application.Logger.addsubStep(LogStatus.FAIL, "PDF window has not opened for literature tab", true);
	...
	public boolean validatePatentPublicationYearChart(Controller Application, HashMap<String, String> input) {
		...
		try {
			...
			page_ChemicalSearchResults.clickOnMoreFiltersDropdown();
			Application.waitTime(2);
			//page_ChemicalSearchResults.clickOnChemicalStructureFilterCheckbox("Publication year", 19);
			page_ChemicalSearchResults.ClickOnPubYearFilterCheckbox();
			afterFilter = page_ChemicalSearchResults.getResultsCount();
			if (beforeFilter >= afterFilter) {
				Application.Logger.addsubStep(LogStatus.PASS,
	...
	public boolean validateManageFieldsClearAllRestoreDefaultOptions(Controller Application,
		...
		List<String> expectedManageFieldOptions = new ArrayList<String>(Arrays.asList("Publication number", "Title",
				"Publication date", "Novelty", "Assignee", "Use", "Inventor", "Advantage", "Example","First claim","Keywords"));
	...
	public boolean validateManageFieldsNonDefaultOptions(Controller Application, HashMap<String, String> input) {
		...
		page_ChemicalSearchResults = new Page_ChemicalSearchResults(Application);
		String searchText = input.get("searchtext");
		List<String> fields = new ArrayList<String>(Arrays.asList("Publication date", "Advantage", "First claim","Example","Keywords"));
		List<String> checkBoxStatus = new ArrayList<String>();
		boolean flag = true;
		try {
			Application.Logger.addStep("1.PERFORM A SEARCH WITH A PHARSE AND VERIFY THE RS PAGE",
			...
			Application.Logger.endStep();
			Application.Logger.addStep("2.SELECT PUBLICATION DATE,ADVANTAGE , FIRST CLAIM,EXAMPLE AND KEYWORDS OPTIONS AND VERIFY THE RS",
					"PUBLICATION DATE,ADVANTAGE , FIRST CLAIM,EXAMPLE AND KEYWORDS OPTIONS SHOULD BE DISPLAYED IN RS");
			page_ChemicalSearchResults.clickOnLinkManageFields();
			Application.waitTime(2);
			page_ChemicalSearchResults.manageFields().clickOnButtonClearAll();
			page_ChemicalSearchResults.manageFields().selectManageFieldCheckBoxes(fields);
			...
			if (TestUtil.ListContains(checkBoxStatus, "true")) {
				Application.Logger.addsubStep(LogStatus.PASS,
						"PUBLICATION DATE,ADVANTAGE, FIRST CLAIM,EXAMPLE AND KEYWORDS OPTIONS CHECK BOXES ARE SELECTED", false);
			} else {
				Application.Logger.addsubStep(LogStatus.FAIL,
						"PUBLICATION DATE,ADVANTAGE, FIRST CLAIM,EXAMPLE AND KEYWORDS OPTIONS CHECK BOXES ARE NOT SELECED", true);
			}
			page_ChemicalSearchResults.manageFields().clickOnCloseIcon();
			...
			if (page_ChemicalSearchResults.isDisplayedKeywords(1)) {
				Application.Logger.addsubStep(LogStatus.PASS, "KEYWORDS SECTION IS DISPLAYED IN RESULTS SET PAGE",
						false);
			} else {
				Application.Logger.addsubStep(LogStatus.FAIL,
						"KEYWORDS SECTION IS NOT DISPLAYED IN RESULTS SET PAGE", true);
			}
			Application.Logger.endStep();
			Application.Logger.addStep(
					"3.DESELECT PUBLICATION DATE,ADVANTAGE AND FIRST CLAIM OPTIONS AND VERIFY THE RS",
					"PUBLICATION DATE,ADVANTAGE, FIRST CLAIM,EXAMPLE AND KEYWORDS OPTIONS SHOULD NOT BE DISPLAYED IN RS");
			page_ChemicalSearchResults.clickOnLinkManageFields();
			Application.waitTime(2);
			...
			if (TestUtil.ListContains(checkBoxStatus, "false")) {
				Application.Logger.addsubStep(LogStatus.PASS,
						"PUBLICATION DATE,ADVANTAGE, FIRST CLAIM,EXAMPLE AND KEYWORDS OPTIONS CHECK BOXES ARE NOT SELECTED", false);
			} else {
				Application.Logger.addsubStep(LogStatus.FAIL,
						"PUBLICATION DATE,ADVANTAGE, FIRST CLAIM,EXAMPLE AND KEYWORDS OPTIONS CHECK BOXES ARE SELECED", true);
			}
			page_ChemicalSearchResults.manageFields().clickOnCloseIcon();
			if (!page_ChemicalSearchResults.isDisplayedPublished(1)) {
				Application.Logger.addsubStep(LogStatus.PASS,
			...
			if (!page_ChemicalSearchResults.isDisplayedKeywords(1)) {
				Application.Logger.addsubStep(LogStatus.PASS, "KEYWORDS SECTION IS NOT DISPLAYED IN RESULTS SET PAGE",
						false);
			} else {
				Application.Logger.addsubStep(LogStatus.FAIL,
						"KEYWORDS SECTION IS DISPLAYED IN RESULTS SET PAGE", true);
			}
			Application.Logger.endStep();
		}
	...
	public boolean validateLiteratureMoreLikeThis(Controller Application, HashMap<String, String> input) {
		...
		try {
			...
			Application.waitTime(3);
			if (rsLitTitle.equalsIgnoreCase(moreLikeLitTitle)) {
			...
			page_ChemicalSearchResults.tabLiteratureSearch().clickOnLiteratureRecord(2);
			Application.waitTime(2);
	...
	public boolean validateSavedRecordsFeature(Controller Application, HashMap<String, String> input) {
		...
		try {
			...
			page_SavedRecords.setEditTextFolderName(createdFolderName);
			Application.waitTime(2);
			...
			page_SavedRecords.clickOnButtonEdit();
			Application.waitTime(2);
			page_SavedRecords.setEditTextFolderName(singleFolderName);
			...
			Application.Logger.addStep("9.CLICK ON DELETE OPTION ",
	...
	public boolean validateModifyChemicalStructureModal(Controller Application, HashMap<String, String> input) {
		...
		try {
			...
			if (page_ChemicalSearchResults.getColorOfStructureSerachPillBox().contains("primary")) {
		...
		public boolean validateStructureSearchAndLiteratureTab(Controller Application, HashMap<String, String> input) {
			...
			public boolean validateStructureSearchKeywordsAndPhrase(Controller Application, HashMap<String, String> input) {
				...
				Application.Logger.endStep();
					...
					Application.Logger.endStep();	
					Application.Logger.addStep("4.VERIFY IF THE MODIFIED PHRASE IS DISPLAYED IN THE RS TOP SECTION","MODIFIED PHRASE SHOULD BE DISPLAYED IN THE RS TOP SECTION");
					...
					Application.Logger.endStep();				
					Application.Logger.addStep("5.MODIFY THE BOTTOM SECTION AND VERIFY IF THE RESULTS ARE REFRESHED AFTER ADDING THE KEYWORDS","RESULTS SHOULD BE REFRESHED AFTER ADDING THE KEYWORDS");
					page_ChemicalSearchResults.setKeyWords(finalKeywordList);
					page_ChemicalSearchResults.clickOnSearchIcon();
					Application.waitUntilFectchRecordProgressBarToDisappears();
					Application.waitUntilFectchRecordProgressBarToDisappears();				
					...
					Application.Logger.endStep();				
					Application.Logger.addStep("6.VERIFY IF THE NEWLY ADDED PIILS ARE DISPLAYED IN THE RS BOTTOM PILLS SECTION","NEWLY ADDED PIILS SHOULD BE DISPLAYED IN THE RS BOTTOM PILLS SECTION");
					List<String> getListOfPills = page_ChemicalSearchResults.getListOfPillBoxes();
					for (String pills : getListOfPills) {
						if (pills != null) {
					...
					Application.waitTime(2);
					Application.Logger.endStep();				
					Application.Logger.addStep("7.PERFORM A SUB STRUCTURE SEARCH AND FREE TEXT SEARCH FROM THE LANDING PAGE","RESULTS SET PAGE SHOULD BE DISPLAYED FOR BOTH EXACT STRUCURE SEARCH AND FREE TEXT SEARCH");
					page_ChemicalSearchResults.clickOnChemExpHomePage();
					Application.waitTime(2);
					page_ChemicalSearchLandingPage.setTextSearchTextBox(searchText);
					Application.waitTime(2);
					...
					beforeStructureSearch=page_ChemicalSearchResults.getResultsCount();
					Application.Logger.endStep();				
					Application.Logger.addStep("8.MODIFY THE TOP SECTION AND VERIFY IF THE RESULTS ARE REFRESHED AFTER SEARCHING WITH THE MODIFIED PHRASE","RESULTS SHOULD BE REFRESHED AFTER SEARCHING WITH THE MODIFIED PHRASE");
					page_ChemicalSearchResults.setTextPhrase(secondsearchText);
					page_ChemicalSearchResults.clickOnSearchIcon();			
					Application.waitUntilFectchRecordProgressBarToDisappears();
					Application.waitUntilFectchRecordProgressBarToDisappears();
					...
					Application.Logger.endStep();	
					Application.Logger.addStep("9.VERIFY IF THE MODIFIED PHRASE IS DISPLAYED IN THE RS TOP SECTION","MODIFIED PHRASE SHOULD BE DISPLAYED IN THE RS TOP SECTION");
					actSearchPhrase=page_ChemicalSearchResults.getTextSearchBox();
					if (secondsearchText.equalsIgnoreCase(actSearchPhrase)) {
						Application.Logger.addsubStep(LogStatus.PASS,"MODIFIED PHRASE IS DISPLAYED IN THE RS TOP SECTION",false);
					} else {
					...
					Application.Logger.endStep();				
					Application.Logger.addStep("10.MODIFY THE BOTTOM SECTION AND VERIFY IF THE RESULTS ARE REFRESHED AFTER ADDING THE KEYWORDS","RESULTS SHOULD BE REFRESHED AFTER ADDING THE KEYWORDS");
					page_ChemicalSearchResults.setKeyWords(finalKeywordList);
					page_ChemicalSearchResults.clickOnSearchIcon();
					Application.waitUntilFectchRecordProgressBarToDisappears();
					Application.waitUntilFectchRecordProgressBarToDisappears();				
					...
					Application.Logger.endStep();				
					Application.Logger.addStep("11.VERIFY IF THE NEWLY ADDED PIILS ARE DISPLAYED IN THE RS BOTTOM PILLS SECTION","NEWLY ADDED PIILS SHOULD BE DISPLAYED IN THE RS BOTTOM PILLS SECTION");
					...
					Application.Logger.addStep("12.VERIFY IF THE STRUCTURE SEARCH ICON IS ENABLED IN THE RS PAGE AFTER PERFORMING SEARCH ONLY WITH THE FREE TEXT","STRUCTURE SEARCH ICON SHOULD BE IN ENABLED STATE");
					page_ChemicalSearchResults.clickOnChemExpHomePage();
					Application.waitTime(2);
					page_ChemicalSearchLandingPage.setTextSearchTextBox(searchText);
					Application.waitTime(2);
					...
					if(page_ChemicalSearchResults.isEnabledStructureSearchIcon())
					{
						Application.Logger.addsubStep(LogStatus.PASS,"STRUCTURE SEARCH ICON IS IN ENABLED IN THE RS PAGE",false);
				} else {
					Application.Logger.addsubStep(LogStatus.FAIL,"STRUCTURE SEARCH ICON IS NOT ENABLED IN THE RS PAGE",true);
				}
				Application.Logger.endStep();
...
public boolean vaildateCompanyPeopleSearchRSPage(Controller Application, HashMap<String, String> input) {
	...
	try {
		...
		page_ChemicalSearchResults.clickOnPersonIcon();
		Application.waitTime(2);
		page_ChemicalSearchResults.setTextPersonOrOrganization("KIM");
		page_ChemicalSearchResults.clickOnButtonAdd();
		page_ChemicalSearchResults.clickOnLabel();
		Application.waitTime(2);
		page_ChemicalSearchResults.clickOnSearchIcon();
		Application.waitUntilFectchRecordProgressBarToDisappears();	
		page_ChemicalSearchResults.clickOnPersonOrgPillBox(2);
		page_ChemicalSearchResults.clickOnPersonOrgPillBox(3);
		page_ChemicalSearchResults.clickOnPersonOrgPillBox(3);
		page_ChemicalSearchResults.clickOnSearchIcon();
		Application.waitUntilFectchRecordProgressBarToDisappears();
		if (page_ChemicalSearchResults.checkIfResultsFound()) {
		...
		Application.Logger.addStep("3.VERIFY THE DEFAULT COLOR BLUE DISPLAYED FOR THE PILL IN THE RS PAGE","PILL SHOULD BE BLUE IN COLOR");
		if (page_ChemicalSearchResults.getColorOfPillBox(1).contains("state_0")) {
		...
		Application.Logger.endStep();
		Application.Logger.addStep("4.VERIFY THAT COLOR BLUE PILL IS CHANGED TO GREEN ON CLICK IN RS PAGE","PILL SHOULD BE CHANGED TO GREEN IN COLOR");
		if (page_ChemicalSearchResults.getColorOfPillBox(2).contains("state_1")) {
			Application.Logger.addsubStep(LogStatus.PASS, "PILL BOX CONTAINS THE GREEN  COLOR AS EXPECTED", false);
		} else {
			Application.Logger.addsubStep(LogStatus.FAIL, "PILL BOX DOESNT CONTAINS THE GREEN COLOR AS EXPECTED", true);
		}
		Application.Logger.endStep();
		
		Application.Logger.addStep("5.VERIFY THAT COLOR GREEN PILL IS CHANGED TO RED ON CLICK IN RS PAGE","PILL SHOULD BE CHANGED TO RED IN COLOR");
		if (page_ChemicalSearchResults.getColorOfPillBox(3).contains("state_2")) {
		...
		if (noOfTimesHightedOfGivenTextPerson > 0) {
			Application.Logger.addsubStep(LogStatus.PASS, noOfTimesHightedOfGivenTextPerson
					+ "\t TIME THE GIVEN PERSON \t FUJITA M IS HIGHLIGHTED IN RESULTSET", false);
		} else {
			Application.Logger.addsubStep(LogStatus.FAIL, noOfTimesHightedOfGivenTextPerson
					+ "\t TIME THE GIVEN PERSON \t FUJITA M IS NOT HIGHLIGHTED IN RESULTSET", true);
		}
		
		int noOfTimesHightedOfGivenTextOrg = page_ChemicalSearchResults.tabPatentSearch()
				.getCountOfHighlightedPersonOrganization("TORAY INDUSTRIES INC");
		if (noOfTimesHightedOfGivenTextOrg > 0) {
			Application.Logger.addsubStep(LogStatus.PASS, noOfTimesHightedOfGivenTextOrg
					+ "\t TIME THE GIVEN ORGANIZATION \t TORAY INDUSTRIES INC IS HIGHLIGHTED IN RESULTSET", false);
		} else {
			Application.Logger.addsubStep(LogStatus.FAIL, noOfTimesHightedOfGivenTextOrg
		...
		int noOfTimesHightedOfGivenTextPersonRv = page_ChemicalSearchResults.tabPatentSearch()
				.getCountOfHighlightedPersonOrganization("FUJITA M");
		if (noOfTimesHightedOfGivenTextPersonRv > 0) {
			Application.Logger.addsubStep(LogStatus.PASS, noOfTimesHightedOfGivenTextPersonRv
					+ "\t TIME THE GIVEN PERSON \t FUJITA M IS HIGHLIGHTED IN RECORD VIEW", false);
		} else {
			Application.Logger.addsubStep(LogStatus.FAIL, noOfTimesHightedOfGivenTextPersonRv
					+ "\t TIME THE GIVEN PERSON \t FUJITA M IS NOT HIGHLIGHTED IN RECORD VIEW", true);
		}
		
		int noOfTimesHightedOfGivenTextOrgRv = page_ChemicalSearchResults.tabPatentSearch()
				.getCountOfHighlightedPersonOrganization("TORAY INDUSTRIES INC");
		if (noOfTimesHightedOfGivenTextOrgRv > 0) {
			Application.Logger.addsubStep(LogStatus.PASS, noOfTimesHightedOfGivenTextOrgRv
					+ "\t TIME THE GIVEN ORGANIZATION \t TORAY INDUSTRIES INC IS HIGHLIGHTED IN RECORD VIEW", false);
		} else {
			Application.Logger.addsubStep(LogStatus.FAIL, noOfTimesHightedOfGivenTextOrgRv
...
public boolean vaildateCompanyPeopleSearch(Controller Application, HashMap<String, String> input) {
	...
	try {
		...
		Application.waitTime(2);
		page_ChemicalSearchLandingPage.setTextPersonOrOrganization("CHASE");
		page_ChemicalSearchLandingPage.clickOnButtonAdd();
		page_ChemicalSearchLandingPage.clickOnLabel();
		page_ChemicalSearchLandingPage.clickOnOrganizationIcon();
		Application.waitTime(2);
		page_ChemicalSearchLandingPage.setTextPersonOrOrganization("UNIVERSITY");
		page_ChemicalSearchLandingPage.clickOnButtonAdd();	
		page_ChemicalSearchLandingPage.clickOnLabel();
		page_ChemicalSearchLandingPage.clickOnPersonIcon();
		Application.waitTime(2);
		page_ChemicalSearchLandingPage.setTextPersonOrOrganization("ZHANG");
		page_ChemicalSearchLandingPage.clickOnButtonAdd();
		page_ChemicalSearchLandingPage.clickOnLabel();
		page_ChemicalSearchLandingPage.clickOnOrganizationIcon();
		Application.waitTime(2);
		page_ChemicalSearchLandingPage.setTextPersonOrOrganization("AKRON");
		page_ChemicalSearchLandingPage.clickOnButtonAdd();	
		page_ChemicalSearchLandingPage.clickOnLabel();
		Application.waitTime(2);
		...
		Application.Logger.addStep("4.VERIFY THAT COLOR GREEN PILL IS CHANGED TO RED ON CLICK IN LANDING PAGE","PILL SHOULD BE CHANGED TO RED IN COLOR");
		page_ChemicalSearchLandingPage.clickOnPersonOrgPillBox(3);
		page_ChemicalSearchLandingPage.clickOnPersonOrgPillBox(3);
		if (page_ChemicalSearchLandingPage.getColorOfPersonOrgPillBox(3).contains("state_2")) {
			Application.Logger.addsubStep(LogStatus.PASS, "PILL BOX CONTAINS THE RED  COLOR AS EXPECTED", false);
		} else {
			Application.Logger.addsubStep(LogStatus.FAIL, "PILL BOX DOESNT CONTAINS THE RED COLOR AS EXPECTED", true);			
		}
		...
		Application.Logger.endStep();				
		Application.Logger.addStep("5.VERIFY IF THE ADDED PERSONS OR ORGANIZATION IN THE LANDING PAGE MATCHES WITH PERSONS OR ORGANIZATION IN THE  PATENT RESULTS SET PAGE","PERSONS OR ORGANIZATION IN THE LANDING PAGE SHOULD MATCHE WITH PERSONS OR ORGANIZATION IN PATENT RESULTS SET PAGE");
...
public boolean vaildateCompanyPeoplePhraseStructureSearch(Controller Application, HashMap<String, String> input) {
	...
	try {
		...
		page_ChemicalSearchLandingPage.clickOnPersonIcon();
		Application.waitTime(2);
		page_ChemicalSearchLandingPage.setTextPersonOrOrganization("FOSS");
		page_ChemicalSearchLandingPage.clickOnButtonAdd();	
		page_ChemicalSearchLandingPage.clickOnLabel();
		page_ChemicalSearchLandingPage.clickOnOrganizationIcon();
		Application.waitTime(2);
		page_ChemicalSearchLandingPage.setTextPersonOrOrganization("TECHNOLOGIES");
		page_ChemicalSearchLandingPage.clickOnButtonAdd();	
		page_ChemicalSearchLandingPage.clickOnLabel();
		page_ChemicalSearchLandingPage.clickOnPersonIcon();
		Application.waitTime(2);
		page_ChemicalSearchLandingPage.setTextPersonOrOrganization("CHAMBER" ); 
		page_ChemicalSearchLandingPage.clickOnButtonAdd();
		page_ChemicalSearchLandingPage.clickOnLabel();
		page_ChemicalSearchLandingPage.clickOnOrganizationIcon();
		Application.waitTime(2);
		page_ChemicalSearchLandingPage.setTextPersonOrOrganization("INC");
		page_ChemicalSearchLandingPage.clickOnButtonAdd();		
		page_ChemicalSearchLandingPage.clickOnLabel();
		Application.waitTime(2);				
		List<String> pillsAtLandingPage = page_ChemicalSearchLandingPage.getAllPersonsOrganizationsStructureSearch();
		page_ChemicalSearchLandingPage.clickOnSearchIcon();
		Application.waitUntilFectchRecordProgressBarToDisappears();			
	 if (page_ChemicalSearchResults.checkIfResultsFound()) {
			Application.Logger.addsubStep(LogStatus.PASS, "RESULT SET IS DISPLAYED FOR THE TEXT SEARCH", false);
...
//CHEMEXP-1824
@Test
public boolean vaildateChemicalNameSearch(Controller Application, HashMap<String, String> input) {
	page_ChemicalSearchLandingPage = new Page_ChemicalSearchLandingPage(Application);
	page_ChemicalSearchResults = new Page_ChemicalSearchResults(Application);
	page_SavedRecords = new Page_SavedRecords(Application);
	String searchText = input.get("searchtext");
	...
	boolean flag = true;
	try {
		Application.Logger.addStep("1.PERFORM A CHEMICAL NAME SEARCH FROM THE LANDING PAGE","RESULTS SET PAGE SHOULD BE DISPLAYED ");
		Application.waitTime(2);
		page_ChemicalSearchLandingPage.clickOnStructureSearchIcon();
		page_ChemicalSearchLandingPage.clickOnTabChemicalNameSearch();
		page_ChemicalSearchLandingPage.setTextChemicalNameSearchTextBox("fiber");	
		page_ChemicalSearchLandingPage.clickOnButtonApply();
		Application.waitTime(2);
		page_ChemicalSearchLandingPage.clickOnSearchIcon();
		Application.waitUntilFectchRecordProgressBarToDisappears();			
		...
		Application.Logger.endStep();
		Application.Logger.addStep("2.VERIFY THE DEFAULT COLOR BLUE DISPLAYED FOR THE PILL IN THE RS KEYWORDS SECTION","PILL SHOULD BE BLUE IN COLOR");
		Application.waitTime(2);
		if (page_ChemicalSearchResults.isDisplayedPillBox(1)) {
		...
		Application.Logger.endStep();
		Application.Logger.addStep("3.VERIFY THAT COLOR BLUE BILL IS CHANGED TO GREEN ON CLICK IN RS KEYWORDS SECTION","PILL SHOULD BE CHANGED TO BLUE IN COLOR");
		Application.waitTime(2);
		page_ChemicalSearchResults.clickOnPillBox(1);
		Application.waitTime(2);
		if (page_ChemicalSearchResults.getColorOfPillBox(1).contains("state_1")) {
		...
		Application.Logger.endStep();
		Application.Logger.addStep("4.VERIFY THAT COLOR GREEN BILL IS CHANGED TO RED ON CLICK IN RS KEYWORDS SECTION","PILL SHOULD BE CHANGED TO RED IN COLOR");
		Application.waitTime(2);
		page_ChemicalSearchResults.clickOnPillBox(1);
		Application.waitTime(2);
		if (page_ChemicalSearchResults.getColorOfPillBox(1).contains("state_2")) {
...
// CHEMEXP-1982
@Test
public boolean vaildatePDFFromRecordView(Controller Application, HashMap<String, String> input) {
	page_ChemicalSearchLandingPage = new Page_ChemicalSearchLandingPage(Application);
	page_ChemicalSearchResults = new Page_ChemicalSearchResults(Application);
	boolean flag = true;
	String searchText = input.get("searchtext");
	try {
		...
		Application.Logger.addStep("2.VERIFY THAT PDF WINDOW IS OPENED SUCCESSFULLY FROM THE PATENT RECORD VIEW",
		...
		if (!MainID.equals(ChildID))
			Application.Logger.addsubStep(LogStatus.PASS, "PDF window opened successfully for Patent record view",false);
		else
			Application.Logger.addsubStep(LogStatus.FAIL, "PDF window has not opened  for Patent record view", true);
		Application.Logger.endStep();
		
		Application.Logger.addStep("3.VERIFY THAT PDF WINDOW IS OPENED SUCCESSFULLY FROM THE LITERATURE RECORD VIEW",
				"SEPARATE PDF WINDOW SHOULD BE OPENED AFTER CLICKING ON THE PDF ICON");
		Application.driver.switchTo().window(MainID);
		Application.waitTime(2);
		page_ChemicalSearchResults.clickOnTabLiterature();
		page_ChemicalSearchResults.tabLiteratureSearch().clickOnLiteratureRecord(1);
		...
		if (!MainID.equals(ChildID1) && (!ChildID1.equals(ChildID2)))
			Application.Logger.addsubStep(LogStatus.PASS, "PDF window opened successfully for Literature record view",
					false);
		else
			Application.Logger.addsubStep(LogStatus.FAIL, "PDF window has not opened for Literature record view", true);
...
public boolean vaildateRsSortOptions(Controller Application, HashMap<String, String> input) {
	...
	boolean flag = true;
	try {
		Application.Logger.addStep("1.PERFORM A SEARCH FROM THE LANDING PAGE","RS PAGE SHOULD BE DISPLAYED");
		page_ChemicalSearchLandingPage.setTextSearchTextBox(searchText);
		page_ChemicalSearchLandingPage.clickOnSearchIcon();
		Application.waitUntilFectchRecordProgressBarToDisappears();
		if (page_ChemicalSearchResults.checkIfResultsFound()) {
		...
		Application.Logger.endStep();		
		Application.Logger.addStep("2.VERIFY THE SORT OPTIONS IN THE RS PAGE","ALL THE 4 SORT OPTIONS SHOULD BE DISPLAYED");
		page_ChemicalSearchResults.selectRsSortOptionsFromDropDown("Relevance");
		String dropdownDefaultValue = page_ChemicalSearchResults.getValueFromRsSortOptionsPageDropdown();
		if (dropdownDefaultValue.equals("Relevance"))
			Application.Logger.addsubStep(LogStatus.PASS, "DEFAULT VALUE Relevance IS SHOWN IN THE RS SORT OPTIONS DROP DOWN LIST", false);
		else
			Application.Logger.addsubStep(LogStatus.FAIL,"DEFAULT VALUE Relevance IS NOT SHOWN IN THE RS SORT OPTIONS DROP DOWN LIST", true);
		
		page_ChemicalSearchResults.selectRsSortOptionsFromDropDown("Date: newest first");
		Application.waitUntilFectchRecordProgressBarToDisappears();
		String DatenewestFirst = page_ChemicalSearchResults.getValueFromRsSortOptionsPageDropdown();
		if (DatenewestFirst.equals("Date: newest first"))
			Application.Logger.addsubStep(LogStatus.PASS, "Date: newest first IS SELECTED FROM THE DROP DOWN LIST", false);
		else
			Application.Logger.addsubStep(LogStatus.FAIL, "Date: newest first IS NOT SELECTED FROM THE DROP DOWN LIST", true);

		page_ChemicalSearchResults.selectRsSortOptionsFromDropDown("Date: oldest first");
		Application.waitUntilFectchRecordProgressBarToDisappears();
		String DateOldestFirst = page_ChemicalSearchResults.getValueFromRsSortOptionsPageDropdown();
		if (DateOldestFirst.equals("Date: oldest first"))
			Application.Logger.addsubStep(LogStatus.PASS, "Date: oldest first IS SELECTED FROM THE DROP DOWN LIST", false);
		else
			Application.Logger.addsubStep(LogStatus.FAIL, "Date: oldest first IS NOT SELECTED FROM THE DROP DOWN LIST", true);
		
		page_ChemicalSearchResults.selectRsSortOptionsFromDropDown("Times cited");
		Application.waitUntilFectchRecordProgressBarToDisappears();
		String timesCited = page_ChemicalSearchResults.getValueFromRsSortOptionsPageDropdown();
		if (timesCited.equals("Times cited"))
			Application.Logger.addsubStep(LogStatus.PASS, "Times cited  IS SELECTED FROM THE DROP DOWN LIST", false);
		else
			Application.Logger.addsubStep(LogStatus.FAIL, "Times cited  IS NOT SELECTED FROM THE DROP DOWN LIST", true);
		Application.Logger.endStep();
	} catch (Exception e) {
		Application.Logger.addException(e.getMessage());
		return flag = false;
...
public boolean vaildateChemicalRoleFilterOptions(Controller Application, HashMap<String, String> input) {
	...
	String searchText = input.get("searchtext");
	int beforeFilter,afterFilter;
	List<String> expectedFilterOptions = new ArrayList<String>(Arrays.asList("Known compound","Composition","Use of a single compound","Synthesis","Product defined by its starting materials","Starting material/Intermediate","Removing or purifying agent","Therapeutically active agent","Reagent","Detecting agent","Analysed or detected","New Compound","Catalyst","Substance removed","Excipient"));
	try {
		Application.Logger.addStep("1.PERFORM A SEARCH WITH A PHARSE AND VERIFY THE RS PAGE","RS PAGE SHOULD BE DISPLAYED AFTER THE SUCCESSFUL SEARCH");
		page_ChemicalSearchLandingPage.setTextSearchTextBox(searchText);
		page_ChemicalSearchLandingPage.clickOnSearchIcon();
		...
		Application.Logger.addStep("2.VERIFY THE SMART FILER CHEMICAL ROLE FILTER OPTIONS","ALL THE OPTIONS SHOULD BE DISPLAYED");
		Application.waitTime(2);
		List<String> actualFilterOptions = page_ChemicalSearchResults.getAllFilterOptions("CHEMICAL ROLE");
		boolean blncheck = TestUtil.validateTwoLists(actualFilterOptions, expectedFilterOptions);
		if (blncheck) {
			Application.Logger.addsubStep(LogStatus.PASS, "EXPECTED AND ACTUAL CHEMICAL ROLE FILTERS FIELDS ARE MATCHING",false);
			Application.Logger.addsubStep(LogStatus.INFO,"ACTUAL CHEMICAL ROLE FILTERS FIELDS: " + actualFilterOptions.size() + actualFilterOptions,
					false);
			Application.Logger.addsubStep(LogStatus.INFO, "EXPECTED CHEMICAL ROLE FILTERS FIELDS: "+ expectedFilterOptions.size() + expectedFilterOptions, false);
		} else {
			Application.Logger.addsubStep(LogStatus.FAIL,"EXPECTED AND ACTUAL CHEMICAL ROLE FILTERS FIELDS ARE NOT MATCHING", true);
			Application.Logger.addsubStep(LogStatus.INFO,"ACTUAL CHEMICAL ROLE FILTERS FIELDS: " + actualFilterOptions.size() + actualFilterOptions,
					false);
			Application.Logger.addsubStep(LogStatus.INFO, "EXPECTED CHEMICAL ROLE FILTERS FIELDS: "+ expectedFilterOptions.size() + expectedFilterOptions, false);
		}
		Application.Logger.endStep();
		Application.Logger.addStep("3.VERIFY IF THE SMART FILER CHEMICAL ROLE PERFORM THE FILTER","FILTER SHOULD BE PERFORMED WITH THE CHEMICAL ROLE SMART FILTER");
		beforeFilter = page_ChemicalSearchResults.getResultsCount();
		page_ChemicalSearchResults.clickOnChemicalStructureFilterCheckbox("CHEMICAL ROLE", 1);
		Application.waitUntilFectchRecordProgressBarToDisappears();
		afterFilter = page_ChemicalSearchResults.getResultsCount();
		if (beforeFilter >= afterFilter) {
			Application.Logger.addsubStep(LogStatus.PASS,"RESULT SETS ARE REFRESHED AFTER THE CHEMICAL ROLE SMART FILTER", false);
		} else {
			Application.Logger.addsubStep(LogStatus.FAIL,"RESULT SETS ARE NOT REFRESHED AFTER THE CHEMICAL ROLE SMART FILTER, BEFORE CHEMICAL ROLE FILTER : \t"
							+ beforeFilter + "\t\t AFTER CHEMICAL ROLE FILTER :\t" + afterFilter,
					true);
		}
		Application.Logger.endStep();
	} catch (Exception e) {
...
public boolean validateSearchistoryFuntionality(Controller Application, HashMap<String, String> input) {
	...
	page_ChemicalSearchResults = new Page_ChemicalSearchResults(Application);
	page_SavedRecords = new Page_SavedRecords(Application);
	page_History = new Page_History(Application);
	String searchText = input.get("searchtext");
	String smiley = input.get("smiles");
	int beforeRerun,afterRerun,historyRecCountAfterDelete,historyRecCountBeforeDelete;
	List<String> finalKeywordList = Arrays.asList(input.get("keywords").split(","));
	String actualErrMsg, expectedErrMsg = "No search history available";
	boolean flag = true;
	try {
		Application.Logger.addStep("1.PERFORM A EXACT STRUCTURE SEARCH AND FREE TEXT SEARCH FROM THE LANDING PAGE","RESULTS SET PAGE SHOULD BE DISPLAYED FOR BOTH EXACT STRUCURE SEARCH AND FREE TEXT SEARCH");
		Application.waitTime(2);
		...
		Application.waitUntilFectchRecordProgressBarToDisappears();
		page_History.clickOnButtonRerunIcon();
		Application.waitTime(4);
		page_ChemicalSearchResults.setKeyWords(finalKeywordList);
		Application.waitTime(2);
		page_ChemicalSearchResults.clickOnSearchIcon();
		beforeRerun=page_ChemicalSearchResults.getResultsCount();
		page_ChemicalSearchLandingPage.clickOnLinkHistory();
		Application.waitUntilFectchRecordProgressBarToDisappears();
		page_History.clickOnButtonRerunIcon();
		afterRerun=page_ChemicalSearchResults.getResultsCount();
		page_ChemicalSearchLandingPage.clickOnLinkHistory();
		Application.waitUntilFectchRecordProgressBarToDisappears();				
		if (beforeRerun == afterRerun) {
			Application.Logger.addsubStep(LogStatus.PASS,"RS COUNTS ARE MATCHING BEFORE AND AFTER RE RUNNING THE SEARCH HISTORY", false);
		} else {
			Application.Logger.addsubStep(LogStatus.FAIL,"RS COUNTS ARE NOT MATCHING BEFORE AND AFTER RE RUNNING THE SEARCH HISTORY, BEFORE RE RUN : \t"+ beforeRerun + "\t\t AFTER RE RUN :\t" + afterRerun,true);
		}
		Application.Logger.endStep();		
		Application.Logger.addStep("3.VERIFY THAT HISTORY RECORD DELETED ON CLICK ON THE DELETE ICON","SEARCH HISTORY RECORD SHOULD BE DELETED");
		page_ChemicalSearchLandingPage.clickOnLinkHistory();
		Application.waitUntilFectchRecordProgressBarToDisappears();
		page_History.clickOnGlobalCheckBox();
		historyRecCountBeforeDelete=page_History.getHistoryRecordCount();
		Application.waitTime(2);
		page_History.clickOnRowDeleteButton(1);
		Application.waitTime(2);
		page_History.clickOnButtonConfirmationYes();
		Application.waitTime(2);
		page_ChemicalSearchResults.clickOnChemExpHomePage();
		page_ChemicalSearchLandingPage.clickOnLinkHistory();
		Application.waitUntilFectchRecordProgressBarToDisappears();
		page_History.clickOnGlobalCheckBox();
		historyRecCountAfterDelete=page_History.getHistoryRecordCount();
		Application.waitTime(4);
		if (historyRecCountBeforeDelete>historyRecCountAfterDelete) {
			Application.waitTime(3);
			Application.Logger.addsubStep(LogStatus.PASS,"SEARCH HISTORY RECORD IS DELETED FROM THE HISTORY PAGE", false);
		} else {
			Application.Logger.addsubStep(LogStatus.FAIL,"SEARCH HISTORY RECORD IS NOT DELETED FROM THE HISTORY PAGE, BEFORE DELETING SH : \t"+ historyRecCountBeforeDelete + "\t\t AFTER DELETING SH :\t" + historyRecCountAfterDelete,true);
		}
		Application.Logger.endStep();		
		Application.Logger.addStep("4.VERIFY THE GLOBAL DELETE OPTION FROM THE HISTORY PAGE","ALL THE RECORDS SHOULD BE DELETED FROM THE HISTORY PAGE");
		page_ChemicalSearchResults.clickOnChemExpHomePage();
		Application.waitTime(2);
		page_ChemicalSearchLandingPage.setTextSearchTextBox(searchText);
		Application.waitTime(2);
		...
		Application.waitUntilFectchRecordProgressBarToDisappears();
		page_History.clickOnGlobalCheckBox();
		page_History.clickOnGlobalDeleteButton();
		Application.waitTime(3);
		page_History.clickOnButtonConfirmationYes();
		actualErrMsg = page_History.getTextErrorMessageNoSearchHistory();
		Application.waitTime(3);
		if (actualErrMsg.equals(expectedErrMsg)) {
			Application.waitTime(2);
			Application.Logger.addsubStep(LogStatus.PASS,"ALL THE RECORDS ARE SUCCESSFULLY DELETED FROM THE SEARCH HISTORY PAGE", false);
		} else {
			Application.Logger.addsubStep(LogStatus.FAIL,"ALL THE RECORDS ARE NOT DELETED FROM THE SEARCH HISTORY PAGE, RECORD COUNT AFTER THE GLOBAL DELETE ",true);
		}
		Application.Logger.endStep();
	} catch (Exception e) {
		Application.Logger.addException(e.getMessage());
...
//Literature Citations
	...
	public boolean validateLiteratureCitations(Controller Application, HashMap<String, String> input) {
		...
		page_ChemicalSearchResults = new Page_ChemicalSearchResults(Application);
		String searchText = input.get("searchtext");
		String expCitingTitle, LitRecordViewTitle, expCitedTitle;
		boolean flag = true;
		try {
			Application.Logger.addStep("1.PERFORM A SEARCH WITH A PHARSE AND VERIFY THE RS PAGE",
					"RS PAGE SHOULD BE DISPLAYED AFTER THE SUCCESSFUL SEARCH");
			...
			Application.Logger.endStep();
			Application.Logger.addStep("2.VERIFY THE CITING LITERATURE SECTION IN LITERATURE RECORD VIEW",
					"CITING LITERATURE SECTION SHOULD BE COLLAPSED BY DEFAULT");
			page_ChemicalSearchResults.clickOnTabLiterature();
			page_ChemicalSearchResults.tabLiteratureSearch().clickOnLiteratureRecord(1);
			if (page_ChemicalSearchResults.tabLiteratureSearch().isCollapsedCitingLiterature()) {
				Application.Logger.addsubStep(LogStatus.PASS, "CITING LITERATURE SECTION IS COLLAPSED BY DEFAULT", false);
			} else {
				Application.Logger.addsubStep(LogStatus.FAIL, "CITING LITERATURE SECTION IS NOT COLLAPSED BY DEFAULT",
						true);
			}
			Application.Logger.endStep();
			Application.Logger.addStep("3.VERIFY THE CITED LITERATURE SECTION IN LITERATURE RECORD VIEW",
					"CITED LITERATURE SECTION SHOULD BE COLLAPSED BY DEFAULT");
			if (page_ChemicalSearchResults.tabLiteratureSearch().isCollapsedCitedLiterature()) {
				Application.Logger.addsubStep(LogStatus.PASS, "CITED LITERATURE SECTION IS COLLAPSED BY DEFAULT", false);
			} else {
				Application.Logger.addsubStep(LogStatus.FAIL, "CITED LITERATURE SECTION IS NOT COLLAPSED BY DEFAULT", true);
			}
			Application.Logger.endStep();
			Application.Logger.addStep("4.VERIFY THE CITING LITERATURE SECTION GETS EXPANDED ON CLICK",
					"CITING LITERATURE SECTION SHOULD BE EXPANDED");
			Application.waitTime(2);
			page_ChemicalSearchResults.tabLiteratureSearch().clickOnLinkCitingLiterature();
			Application.waitTime(2);
			if (page_ChemicalSearchResults.tabLiteratureSearch().isExpandedCitingLiterature()) {
				Application.Logger.addsubStep(LogStatus.PASS, "CITING LITERATURE SECTION IS EXPANDED ON CLICK", false);
			} else {
				Application.Logger.addsubStep(LogStatus.FAIL, "CITING LITERATURE SECTION IS NOT EXPANDED ON CLICK", true);
			}
			Application.Logger.endStep();

			Application.Logger.addStep("5.VERIFY CITED LITERATURE SECTION GETS EXPANDED ON CLICK",
					"CITED LITERATURE SECTION SHOULD BE EXPANDED");
			page_ChemicalSearchResults.tabLiteratureSearch().clickOnLinkCitedLiterature();
			if (page_ChemicalSearchResults.tabLiteratureSearch().isExpandedCitedLiterature()) {
				Application.Logger.addsubStep(LogStatus.PASS, "CITED LITERATURE SECTION IS EXPANDED ON CLICK", false);
			} else {
		...
		/*	Application.Logger.addStep(
					...
					"RECORD VIEW SHOULD BE PAINTED BACK FOR THE SELECTED CITING TITLE");
			if (page_ChemicalSearchResults.tabLiteratureSearch().isExpandedCitingLiterature()) {
				expCitingTitle = page_ChemicalSearchResults.tabLiteratureSearch().getTextCitingTitle();
				page_ChemicalSearchResults.tabLiteratureSearch().clickOnCitingLiteratureTitle();
				LitRecordViewTitle = page_ChemicalSearchResults.tabLiteratureSearch().getTextLiteratureRecordViewTitle();
				if (expCitingTitle.equals(LitRecordViewTitle)) {
					Application.Logger
							.addsubStep(LogStatus.PASS,
									"CITING LITERATURE TITLE : \t\t" + expCitingTitle
											+ "\t\t IS MATCHING WITH RECORD VIEW LITERATURE TITLE :\t" + LitRecordViewTitle,
									false);
				} else {
					Application.Logger.addsubStep(LogStatus.FAIL,
							"CITING LITERATURE TITLE : \t\t" + expCitingTitle
									+ "\t\t IS NOT MATCHING WITH RECORD VIEW LITERATURE TITLE :\t" + LitRecordViewTitle,
							true);
				}

			} else {
				Application.Logger.addsubStep(LogStatus.FAIL,
						"CITING LITERATURE TITLE IS NOT AVAILABLE IN CITING LITERATURE SECTION", false);
			}
			Application.Logger.endStep();

			Application.Logger.addStep(
					"7.VERIFY THAT CLICK ON CITED TITLE FROM CITED LITERATURE SECTION PAINTS THE RECORD VIEW FOR THE SELECTED TITLE",
					"RECORD VIEW SHOULD BE PAINTED BACK FOR THE SELECTED CITED TITLE");
			Application.waitTime(2);
			if (page_ChemicalSearchResults.tabLiteratureSearch().isExpandedCitedLiterature()) {
				Application.waitTime(2);
				expCitedTitle = page_ChemicalSearchResults.tabLiteratureSearch().getTextCitedTitle();
				page_ChemicalSearchResults.tabLiteratureSearch().clickOnCitedLiteratureTitle();
				LitRecordViewTitle = page_ChemicalSearchResults.tabLiteratureSearch().getTextLiteratureRecordViewTitle();
				if (expCitedTitle.equals(LitRecordViewTitle)) {
					Application.Logger
							.addsubStep(LogStatus.PASS,
									"CITED LITERATURE TITLE : \t\t" + expCitedTitle
											+ "\t\t IS MATCHING WITH RECORD VIEW LITERATURE TITLE :\t" + LitRecordViewTitle,
									false);
				} else {
					Application.Logger.addsubStep(LogStatus.FAIL,
			...
			page_ChemicalSearchResults.tabLiteratureSearch().clickOnLiteratureRecord(4);
			Application.waitTime(2);
			if (page_ChemicalSearchResults.tabLiteratureSearch().isExpandedCitingLiterature()) {
				Application.waitTime(2);
				Application.Logger.addsubStep(LogStatus.PASS, "CITING LITERATURE SECTION REMAINS IN EXPANDED STATE",
						false);
			} else {
				Application.Logger.addsubStep(LogStatus.FAIL, "CITING LITERATURE SECTION IS NOT IN EXPANDED STATE", true);
			}
			Application.Logger.endStep();
			Application.Logger.addStep(
					"9.VERIFY THE CITED LITERATURE SECTION REMAINS IN EXPANDED STATE WHILE ACCESSING ANOTHER LITERATURE RECORD",
					"CITED LITERATURE SECTION SHOULD BE IN EXPANDED STATE");
			if (page_ChemicalSearchResults.tabLiteratureSearch().isExpandedCitedLiterature()) {
				Application.Logger.addsubStep(LogStatus.PASS, "CITED LITERATURE SECTION REMAINS IN EXPANDED STATE", false);
			} else {
				Application.Logger.addsubStep(LogStatus.FAIL, "CITED LITERATURE SECTION IS NOT IN EXPANDED STATE", true);
			}
			Application.Logger.endStep();
		} catch (Exception e) {
			Application.Logger.addException(e.getMessage());
	...
	//Literature ViewAsResultSet
		...
		public boolean validateLiteratureViewAsResultsSet(Controller Application, HashMap<String, String> input) {
			...
			page_ChemicalSearchResults = new Page_ChemicalSearchResults(Application);
			String searchText = input.get("searchtext");
			String viewAsResultSetTitle, LiteratureTitle;
			int afterViewAsResultFilter, beforeCitingLiteraturefilter, afterCitingLiteraturefilter, afterCitedLiteraturefilter;
			boolean flag = true;
			try {
				Application.Logger.addStep("1.PERFORM A SEARCH WITH A PHARSE AND VERIFY THE RS PAGE",
						"RS PAGE SHOULD BE DISPLAYED AFTER THE SUCCESSFUL SEARCH");
				...
				Application.Logger.addStep(
						...
						"RS PAGE SHOULD BE REFRESHED AFTER THE CLICK ON VIEW AS RESULT SET BUTTON");
				page_ChemicalSearchResults.clickOnTabLiterature();
				beforeCitingLiteraturefilter = page_ChemicalSearchResults.getResultsCount();
				page_ChemicalSearchResults.tabLiteratureSearch().clickOnLiteratureRecord(1);
				Application.waitTime(1);
				LiteratureTitle = page_ChemicalSearchResults.tabLiteratureSearch().getTextLiteratureRecordViewTitle();
				page_ChemicalSearchResults.tabLiteratureSearch().clickOnLinkCitingLiterature();
				page_ChemicalSearchResults.tabLiteratureSearch().clickOnCitingLiteratureViewASResultSet();
				Application.waitUntilFectchRecordProgressBarToDisappears();
				afterCitingLiteraturefilter = page_ChemicalSearchResults.getResultsCount();
				if (beforeCitingLiteraturefilter >= afterCitingLiteraturefilter) {
					Application.Logger.addsubStep(LogStatus.PASS,
							"RESULT SETS ARE REFRESHED AFTER THE CLICK ON VIEW AS RESULT SET BUTTON FROM CITING LITERATURE SECTION",
							false);
				} else {
					Application.Logger.addsubStep(LogStatus.FAIL,
							"RESULT SETS ARE NOT REFRESHED AFTER THE CLICK ON VIEW AS RESULT SET BUTTON FROM CITING LITERATURE SECTION : \t"
									+ beforeCitingLiteraturefilter + "\t\t AFTER VIEW AS RESULT SET FILTER :\t"
									+ afterCitingLiteraturefilter,true);
				}
				viewAsResultSetTitle = page_ChemicalSearchResults.tabLiteratureSearch().getTextKeyWordMoreLike();
				Application.waitTime(2);
				if (viewAsResultSetTitle.contains("Literature citing")
						&& (viewAsResultSetTitle.contains(LiteratureTitle))) {
					Application.Logger.addsubStep(LogStatus.PASS, "USER IS IN VIEW AS RESULTS SET : LITERATURE CITING MODE",
							false);
				} else {
					Application.Logger.addsubStep(LogStatus.FAIL,
							"USER IS NOT IN VIEW AS RESULTS SET : LITERATURE CITING MODE", true);
				}
				Application.Logger.endStep();
				Application.Logger.addStep(
						"3.VERIFY THE LITERATURE RS IS REFRESHED AFTER THE CLICK ON VIEW AS RESULT SET BUTTON FROM CITED LITERATURE SECTION",
						"RS PAGE SHOULD BE REFRESHED AFTER THE CLICK ON VIEW AS RESULT SET BUTTON");
				page_ChemicalSearchResults.tabLiteratureSearch().clickOnLiteratureRecord(1);
				Application.waitTime(1);
				LiteratureTitle = page_ChemicalSearchResults.tabLiteratureSearch().getTextLiteratureRecordViewTitle();
				page_ChemicalSearchResults.tabLiteratureSearch().clickOnLinkCitedLiterature();
				Application.waitTime(2);
				page_ChemicalSearchResults.tabLiteratureSearch().clickOnCitedLiteratureViewASResultSet();
				Application.waitUntilFectchRecordProgressBarToDisappears();
				afterCitedLiteraturefilter = page_ChemicalSearchResults.getResultsCount();
				if (beforeCitingLiteraturefilter >= afterCitedLiteraturefilter) {
					Application.Logger.addsubStep(LogStatus.PASS,
							"RESULT SETS ARE REFRESHED AFTER THE CLICK ON VIEW AS RESULT SET BUTTON FROM CITED LITERATURE SECTION",
							false);
				} else {
					Application.Logger.addsubStep(LogStatus.FAIL,
							"RESULT SETS ARE NOT REFRESHED AFTER THE CLICK ON VIEW AS RESULT SET BUTTON FROM CITED LITERATURE SECTION : \t"
									+ beforeCitingLiteraturefilter + "\t\t AFTER VIEW AS RESULT SET FILTER :\t"
									+ afterCitedLiteraturefilter, true);
				}
				viewAsResultSetTitle = page_ChemicalSearchResults.tabLiteratureSearch().getTextKeyWordMoreLike();
				Application.waitTime(2);
				if (viewAsResultSetTitle.contains("Literature cited")
						&& (viewAsResultSetTitle.contains(LiteratureTitle))) {
					Application.Logger.addsubStep(LogStatus.PASS, "USER IS IN VIEW AS RESULTS SET : LITERATURE CITED MODE",
							false);
				} else {
					Application.Logger.addsubStep(LogStatus.FAIL, "USER IS NOT IN VIEW AS RESULTS SET : LITERATURE CITED MODE",
							true);
				}
				Application.Logger.endStep();
				Application.Logger.addStep("4.VERIFY THE KEYWORD SECTION IS DISABLED IN VIEW AS RESULTS SET MODE",
				...
				page_ChemicalSearchResults.tabLiteratureSearch().clickOnCloseIconMoreLike();
				afterViewAsResultFilter = page_ChemicalSearchResults.getResultsCount();
				if (beforeCitingLiteraturefilter == afterViewAsResultFilter) {
					Application.Logger.addsubStep(LogStatus.PASS,
							"RESULT SETS ARE REFRESHED AND USER IS OUT OF VIEW AS RESULTS SET MODE", false);
				} else {
					Application.Logger
							.addsubStep(LogStatus.FAIL,
									"RESULT SETS ARE NOT REFRESHED AFTER CLOSING THE VIEW AS RESULTS SET : \t"
											+ beforeCitingLiteraturefilter + "\t\t AFTER FILTER :\t" + afterViewAsResultFilter,
									true);
				}
				Application.Logger.endStep();
			} catch (Exception e) {
